...
from tensorflow.keras.layers import Input, concatenate, Conv2D, MaxPooling2D, UpSampling2D, Reshape, Dropout, TimeDistributed
...
def BCDU_net_D3(input_size=(100, 256, 256, 1)):
    num_slices = input_size[0]
    N = input_size[1]
    ...
    # D2
    conv4_2 = TimeDistributed(Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer =
        'he_normal'))(drop4_1)     
    ...
    conv4_3 = TimeDistributed(Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer =
    ...
    up6 = TimeDistributed(Conv2DTranspose(256, kernel_size=2, strides=2, padding='same',kernel_initializer =
        'he_normal'))(drop4_3)
    up6 = BatchNormalization(axis=-1)(up6)
    up6 = Activation('relu')(up6)

    x1 = Reshape(target_shape=(num_slices, 1, np.int32(N/4), np.int32(N/4), 256))(drop3)
    x2 = Reshape(target_shape=(num_slices, 1, np.int32(N/4), np.int32(N/4), 256))(up6)
    merge6  = concatenate([x1,x2], axis=-1) 
    merge6 = TimeDistributed(ConvLSTM2D(filters = 128, kernel_size=(3, 3), padding='same', return_sequences = False,
    ...
    x1 = Reshape(target_shape=(num_slices, 1, np.int32(N/2), np.int32(N/2), 128))(conv2)
    x2 = Reshape(target_shape=(num_slices, 1, np.int32(N/2), np.int32(N/2), 128))(up7)
    merge7  = concatenate([x1,x2], axis=-1) 
    merge7 = TimeDistributed(ConvLSTM2D(filters = 64, kernel_size=(3, 3), padding='same', return_sequences = False,
    ...
    up8 = TimeDistributed(Conv2DTranspose(64, kernel_size=2, strides=2, padding='same',kernel_initializer =
        'he_normal'))(conv7)
    up8 = BatchNormalization(axis=-1)(up8)
    up8 = Activation('relu')(up8)    

    x1 = Reshape(target_shape=(num_slices, 1, N, N, 64))(conv1)
    x2 = Reshape(target_shape=(num_slices, 1, N, N, 64))(up8)
    merge8  = concatenate([x1,x2], axis=-1) 
    merge8 = TimeDistributed(ConvLSTM2D(filters = 32, kernel_size=(3, 3), padding='same', return_sequences = False,
